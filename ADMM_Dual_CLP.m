...
for iter = 1:1000,
    ...
    for i = 1:n,
        S(:,:,i) = (1/rho)*X(:,:,i) + [gamma(i,1) y(1,i) - x(:,i)'*beta; 
                                       y(1,i) - x(:,i)'*beta z(:,i)'*alpha];
        [V, D] = eig(S(:,:,i));
        for j = 1:2,
           if D(j,j) < 0,
                D(j,j) = 0;
...
figure();
hold on;
h1 = plot(1:iter, opt_val_store(1:iter));
h2 = plot(1:iter, flag_count, 'r');
hXLabel = xlabel('Iter');
hLegend = legend('||f(x^*) - f(x_k)||', '# unsatisfied');
hTitle = title('Normal ADMM');
